...
*
* Note: Main JavaScript file for the CatMarkDownEditor.
*
*/
...
function createToolbarDropdown(options, enableTooltips, shortcuts, parent) {
    ...
    el.classList.add('catmarkdowneditor-dropdown');
    ...
    content.className = 'catmarkdowneditor-dropdown-content';
 ...
 * @param {CatMarkDownEditor} editor
 */
function toggleFullScreen(editor) {
    ...
    if (sidebyside.classList.contains('editor-preview-active-side')) {
        if (editor.options.sideBySideFullscreen === false) {
            ...
            var catMarkDownEditorContainer = wrapper.parentNode;
            if (cm.getOption('fullScreen')) {
                catMarkDownEditorContainer.classList.remove('sided--no-fullscreen');
            } else {
                catMarkDownEditorContainer.classList.add('sided--no-fullscreen');
            }
        } else {
...
function toggleCodeBlock(editor) {
    ...
    if (is_code === 'single') {
        // similar to some CatMarkDownEditor _toggleBlock logic
 ...
 * @param {CatMarkDownEditor} editor The CatMarkDownEditor object
 */
    ...
    for (var i = startPoint.line; i <= endPoint.line; i++) {
        (function (i) {
            ...
            if (stat[name]) {
                ...
                // Workaround for https://github.com/Ionaru/catmarkdowneditor/issues/92
...
/**
 * Interface of CatMarkDownEditor.
 */
function CatMarkDownEditor(options) {
    ...
    if (options.element) {
        ...
        console.log('CatMarkDownEditor: Error. No element was found.');
        return;
    }
 ...
 * @param [onSuccess] {function} see CatMarkDownEditor.prototype.uploadImage
 * @param [onError] {function} see CatMarkDownEditor.prototype.uploadImage
 */
CatMarkDownEditor.prototype.uploadImages = function (files, onSuccess, onError) {
    if (files.length === 0) {
        return;
    }
    var names = [];
 ...
 * @param {FileList} files The files to upload the the server.
 */
CatMarkDownEditor.prototype.uploadImagesUsingCustomFunction = function (imageUploadFunction, files) {
    if (files.length === 0) {
        return;
    }
    var names = [];
 ...
 */
CatMarkDownEditor.prototype.updateStatusBar = function (itemName, content) {
    ...
    if (matchingClasses.length === 1) {
        ...
        console.log('CatMarkDownEditor: status bar item ' + itemName + ' was not found.');
    } else {
        console.log('CatMarkDownEditor: Several status bar items named ' + itemName + ' was found.');
    }
};
 ...
 */
CatMarkDownEditor.prototype.markdown = function (text) {
    if (marked) {
        // Initialize
        var markedOptions;
 ...
 */
CatMarkDownEditor.prototype.render = function (el) {
    if (!el) {
    ...
    this.codemirror = CodeMirror.fromTextArea(el, {
        mode: mode,
        backdrop: backdrop,
        theme: (options.theme != undefined) ? options.theme : 'catmarkdowneditor',
    ...
    var catMarkDownEditorContainer = document.createElement('div');
    catMarkDownEditorContainer.classList.add('CatMarkDownEditorContainer');
    catMarkDownEditorContainer.setAttribute('role', 'application');
    var cmWrapper = this.codemirror.getWrapperElement();
    cmWrapper.parentNode.insertBefore(catMarkDownEditorContainer, cmWrapper);
    catMarkDownEditorContainer.appendChild(cmWrapper);
    ...
    function handleImages() {
        ...
        catMarkDownEditorContainer.querySelectorAll('.cm-image-marker').forEach(function (e) {
...
CatMarkDownEditor.prototype.cleanup = function () {
...
function isLocalStorageAvailable() {
    if (typeof localStorage === 'object') {
        try {
            localStorage.setItem('catmarkdowneditor_localStorage', 1);
            localStorage.removeItem('catmarkdowneditor_localStorage');
        } catch (e) {
            return false;
        }
    } else {
...
CatMarkDownEditor.prototype.autosave = function () {
    if (isLocalStorageAvailable()) {
        var catMarkDownEditor = this;

        if (this.options.autosave.uniqueId == undefined || this.options.autosave.uniqueId == '') {
            console.log('CatMarkDownEditor: You must set a uniqueId to use the autosave feature');
            return;
        }

        if (this.options.autosave.binded !== true) {
            if (catMarkDownEditor.element.form != null && catMarkDownEditor.element.form != undefined) {
                catMarkDownEditor.element.form.addEventListener('submit', function () {
                    clearTimeout(catMarkDownEditor.autosaveTimeoutId);
                    catMarkDownEditor.autosaveTimeoutId = undefined;

                    localStorage.removeItem('catmarkdowneditor_' + catMarkDownEditor.options.autosave.uniqueId);
                });
            }
        ...
        if (this.options.autosave.loaded !== true) {
            if (typeof localStorage.getItem('catmarkdowneditor_' + this.options.autosave.uniqueId) == 'string' && localStorage.getItem('catmarkdowneditor_' + this.options.autosave.uniqueId) != '') {
                this.codemirror.setValue(localStorage.getItem('catmarkdowneditor_' + this.options.autosave.uniqueId));
        ...
        var value = catMarkDownEditor.value();
        if (value !== '') {
            localStorage.setItem('catmarkdowneditor_' + this.options.autosave.uniqueId, value);
        } else {
            localStorage.removeItem('catmarkdowneditor_' + this.options.autosave.uniqueId);
        }
        ...
        console.log('CatMarkDownEditor: localStorage not available, cannot autosave');
    }
};

CatMarkDownEditor.prototype.clearAutosavedValue = function () {
    if (isLocalStorageAvailable()) {
        if (this.options.autosave == undefined || this.options.autosave.uniqueId == undefined || this.options.autosave.uniqueId == '') {
            console.log('CatMarkDownEditor: You must set a uniqueId to clear the autosave value');
            return;
        }
 ...
 * @param [onError] {function} see CatMarkDownEditor.prototype.uploadImage
 */
CatMarkDownEditor.prototype.openBrowseFileWindow = function (onSuccess, onError) {
    var self = this;
 ...
 */
CatMarkDownEditor.prototype.uploadImage = function (file, onSuccess, onError) {
    var self = this;
    ...
    request.onload = function () {
        try {
            ...
            console.error('CatMarkDownEditor: The server did not return a valid json.');
        ...
        if (this.status === 200 && response && !response.error && response.data && response.data.filePath) {
            ...
            if (response.error && response.error in self.options.errorMessages) {  // preformatted error message
                ...
                console.error('CatMarkDownEditor: Received an unexpected response after uploading the image.'
    ...
    request.onerror = function (event) {
        console.error('CatMarkDownEditor: An unexpected error occurred when trying to upload the image.'
 ...
 * @param file {File} The image to upload, as a HTML5 File object (https://developer.mozilla.org/en-US/docs/Web/API/File).
 */
CatMarkDownEditor.prototype.uploadImageUsingCustomFunction = function (imageUploadFunction, file) {
    var self = this;
...
CatMarkDownEditor.prototype.setPreviewMaxHeight = function () {
    var cm = this.codemirror;
    var wrapper = cm.getWrapperElement();
    var preview = wrapper.nextSibling;
...
CatMarkDownEditor.prototype.createSideBySide = function () {
    var cm = this.codemirror;
    var wrapper = cm.getWrapperElement();
    var preview = wrapper.nextSibling;
...
CatMarkDownEditor.prototype.createToolbar = function (items) {
...
CatMarkDownEditor.prototype.createStatusbar = function (status) {
    // Initialize
 ...
 */
CatMarkDownEditor.prototype.value = function (val) {
    var cm = this.codemirror;
    if (val === undefined) {
        return cm.getValue();
    } else {
 ...
 */
CatMarkDownEditor.toggleBold = toggleBold;
CatMarkDownEditor.toggleItalic = toggleItalic;
CatMarkDownEditor.toggleStrikethrough = toggleStrikethrough;
CatMarkDownEditor.toggleBlockquote = toggleBlockquote;
CatMarkDownEditor.toggleHeadingSmaller = toggleHeadingSmaller;
CatMarkDownEditor.toggleHeadingBigger = toggleHeadingBigger;
CatMarkDownEditor.toggleHeading1 = toggleHeading1;
CatMarkDownEditor.toggleHeading2 = toggleHeading2;
CatMarkDownEditor.toggleHeading3 = toggleHeading3;
CatMarkDownEditor.toggleHeading4 = toggleHeading4;
CatMarkDownEditor.toggleHeading5 = toggleHeading5;
CatMarkDownEditor.toggleHeading6 = toggleHeading6;
CatMarkDownEditor.toggleCodeBlock = toggleCodeBlock;
CatMarkDownEditor.toggleUnorderedList = toggleUnorderedList;
CatMarkDownEditor.toggleOrderedList = toggleOrderedList;
CatMarkDownEditor.cleanBlock = cleanBlock;
CatMarkDownEditor.drawLink = drawLink;
CatMarkDownEditor.drawImage = drawImage;
CatMarkDownEditor.drawUploadedImage = drawUploadedImage;
CatMarkDownEditor.drawTable = drawTable;
CatMarkDownEditor.drawHorizontalRule = drawHorizontalRule;
CatMarkDownEditor.undo = undo;
CatMarkDownEditor.redo = redo;
CatMarkDownEditor.togglePreview = togglePreview;
CatMarkDownEditor.toggleSideBySide = toggleSideBySide;
CatMarkDownEditor.toggleFullScreen = toggleFullScreen;

/**
 * Bind instance methods for exports.
 */
CatMarkDownEditor.prototype.toggleBold = function () {
    toggleBold(this);
};
CatMarkDownEditor.prototype.toggleItalic = function () {
    toggleItalic(this);
};
CatMarkDownEditor.prototype.toggleStrikethrough = function () {
    toggleStrikethrough(this);
};
CatMarkDownEditor.prototype.toggleBlockquote = function () {
    toggleBlockquote(this);
};
CatMarkDownEditor.prototype.toggleHeadingSmaller = function () {
    toggleHeadingSmaller(this);
};
CatMarkDownEditor.prototype.toggleHeadingBigger = function () {
    toggleHeadingBigger(this);
};
CatMarkDownEditor.prototype.toggleHeading1 = function () {
    toggleHeading1(this);
};
CatMarkDownEditor.prototype.toggleHeading2 = function () {
    toggleHeading2(this);
};
CatMarkDownEditor.prototype.toggleHeading3 = function () {
    toggleHeading3(this);
};
CatMarkDownEditor.prototype.toggleHeading4 = function () {
    toggleHeading4(this);
};
CatMarkDownEditor.prototype.toggleHeading5 = function () {
    toggleHeading5(this);
};
CatMarkDownEditor.prototype.toggleHeading6 = function () {
    toggleHeading6(this);
};
CatMarkDownEditor.prototype.toggleCodeBlock = function () {
    toggleCodeBlock(this);
};
CatMarkDownEditor.prototype.toggleUnorderedList = function () {
    toggleUnorderedList(this);
};
CatMarkDownEditor.prototype.toggleOrderedList = function () {
    toggleOrderedList(this);
};
CatMarkDownEditor.prototype.cleanBlock = function () {
    cleanBlock(this);
};
CatMarkDownEditor.prototype.drawLink = function () {
    drawLink(this);
};
CatMarkDownEditor.prototype.drawImage = function () {
    drawImage(this);
};
CatMarkDownEditor.prototype.drawUploadedImage = function () {
    drawUploadedImage(this);
};
CatMarkDownEditor.prototype.drawTable = function () {
    drawTable(this);
};
CatMarkDownEditor.prototype.drawHorizontalRule = function () {
    drawHorizontalRule(this);
};
CatMarkDownEditor.prototype.undo = function () {
    undo(this);
};
CatMarkDownEditor.prototype.redo = function () {
    redo(this);
};
CatMarkDownEditor.prototype.togglePreview = function () {
    togglePreview(this);
};
CatMarkDownEditor.prototype.toggleSideBySide = function () {
    toggleSideBySide(this);
};
CatMarkDownEditor.prototype.toggleFullScreen = function () {
    toggleFullScreen(this);
};

CatMarkDownEditor.prototype.isPreviewActive = function () {
    var cm = this.codemirror;
    var wrapper = cm.getWrapperElement();
    var preview = wrapper.lastChild;
...
CatMarkDownEditor.prototype.isSideBySideActive = function () {
    var cm = this.codemirror;
    var wrapper = cm.getWrapperElement();
    var preview = wrapper.nextSibling;
...
CatMarkDownEditor.prototype.isFullscreenActive = function () {
    var cm = this.codemirror;

    return cm.getOption('fullScreen');
};

CatMarkDownEditor.prototype.getState = function () {
    var cm = this.codemirror;

    return getState(cm);
};

CatMarkDownEditor.prototype.toTextArea = function () {
    var cm = this.codemirror;
    var wrapper = cm.getWrapperElement();
    var catMarkDownEditorContainer = wrapper.parentNode;

    if (catMarkDownEditorContainer) {
        if (this.gui.toolbar) {
            catMarkDownEditorContainer.removeChild(this.gui.toolbar);
        }
        if (this.gui.statusbar) {
            catMarkDownEditorContainer.removeChild(this.gui.statusbar);
        }
        if (this.gui.sideBySide) {
            catMarkDownEditorContainer.removeChild(this.gui.sideBySide);
        }
    }

    // Unwrap catMarkDownEditorcontainer before codemirror toTextArea() call
    catMarkDownEditorContainer.parentNode.insertBefore(wrapper, catMarkDownEditorContainer);
    catMarkDownEditorContainer.remove();
...
module.exports = CatMarkDownEditor;
